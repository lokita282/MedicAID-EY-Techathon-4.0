...
export default function UtilityModal({
  ...
  const redeemvou = async () => {
    ...
    if (res.data.success) {
      ...
      await getCategoryCoupon(category).then((res) => {
      ...
      >
        <Box sx={style}>
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            <CardMedia
              component="img"
              image={solo?.issuedByLogo}
              sx={{ width: '50px', borderRadius: '50px' }}
            />
            <CardMedia
              component="img"
              image={solo?.orgLogo}
          ...
          {loading ? (
            ...
            >
              ...
              <p style={ptag}>
                {solo?.useType === 'multiple' ? 'Multiple Use' : 'Single Use'} |{' '}
                <b>
                  ₹{' '}
                  {solo?.useType === 'multiple'
                    ? solo?.balanceAmount
                    : solo?.amount}
                </b>
              </p>
              {!click ? (
                <>
                  <Typography
                    variant="h5"
                    sx={{ ...bold_name, textAlign: 'center', marginTop: '10%' }}
                  >
                    Would you like to continue and redeem the voucher?
                  </Typography>
                ...
                <>
                  <Typography sx={{ ...ptag, marginTop: '10%' }}>
                    We have sent verificationcode on xxx
                    {JSON.parse(localStorage.getItem('eyUser')).phone.slice(
                      7,
                      10
                    )}
                  </Typography>
            ...
            >
              ...
              <Typography sx={{ ...ptag, color: '#73AF55', marginTop: '5%' }}>
