...
export default function SignupBene() {
  ...
  const { setUser, setToken } = useContext(eycontext)
    ...
    if (json.name && json.aadhar && json.phone && json.password && json.pan) {
      await signup(json)
        .then((res) => {
          ...
          localStorage.setItem('eyToken', res.data.data.token)
          localStorage.setItem('eyUser', JSON.stringify(res.data.data.user))
